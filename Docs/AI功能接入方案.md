...
| 方案 | 优势 | 劣势 | 适用场景 | 成本 |
|------|------|------|----------|------|
| **OpenAI GPT-4 + Vision** | 功能强大、多模态、质量高 | 需要网络、API调用成本 | 笔记生成、游记生成、描述生成 | 中等-高 |
| **Apple Core ML** | 本地运行、隐私好、免费 | 功能有限、需训练模型 | 照片分类、简单标签 | 低（开发成本高） |
| **Apple Intelligence (iOS 18+)** | 系统集成、隐私优先 | 仅支持新系统、功能受限 | 系统级AI功能 | 免费 |
| **Google Gemini** | 多模态能力强、免费额度 | 需要网络、隐私考虑 | 图像分析、文本生成 | 低-中等 |
...
#### 阶段一：使用 OpenAI API（快速验证）
- **功能**：智能笔记生成、旅程描述生成
- **优势**：开发快速、功能强大、易于迭代
- **成本**：按使用量付费（$0.01-0.03/请求）
- **实现**：封装 API 服务层，便于后续替换
...
│   ├── OpenAIProvider.swift             # OpenAI API实现
│   ├── AppleIntelligenceProvider.swift  # Apple Intelligence实现
...
#### 实现要点
- **图像识别API**：使用 GPT-4 Vision 或 Apple Vision 框架
- **识别内容**：
  - 场景类型（自然、城市、建筑、美食等）
  - 主要物体（地标、活动等）
  - 照片质量评估
...
2. **环境配置**
   - [ ] 创建 OpenAI API Key 管理（使用环境变量或配置）
   - [ ] 添加网络请求封装
   - [ ] 实现错误处理和重试机制
...
#### OpenAI API 成本（参考）
- **GPT-4 Vision (图片+文本)**：
  - 图片分析：$0.01-0.03/张（取决于分辨率）
  - 文本生成：$0.03-0.06/千token
- **GPT-4 Turbo (纯文本)**：
  - $0.01/千token (输入)
  - $0.03/千token (输出)

**示例计算**：
- 单次笔记生成（1张照片 + 300字生成）：约 $0.05
- 旅程描述生成（10个目的地分析 + 500字生成）：约 $0.15
- **月成本预估**（假设100次笔记生成 + 20次旅程描述）：约 $8-10
...
class AIModelManager: ObservableObject {
    ...
    init(service: AIServiceProtocol = OpenAIService()) {
...
**如果你需要**：
- ✅ 快速上线 → OpenAI API
- ✅ 用户隐私优先 → Apple Intelligence (iOS 18+) + 降级方案
- ✅ 离线功能 → Core ML 自定义模型
- ✅ 成本控制 → Google Gemini（有免费额度）

**如果预算有限**：
1. 先使用免费额度测试（OpenAI/Google都有）
2. 实现用户开关控制使用量
3. 逐步优化，降低单次调用成本

**如果想平衡**：
→ 推荐混合方案：Apple Intelligence（首选）+ OpenAI（降级）
