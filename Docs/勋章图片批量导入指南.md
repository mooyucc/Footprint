...
**在添加新国家的勋章图片之前，必须先更新 `CountryManager.swift` 文件！**

### 为什么需要先更新代码？

勋章列表是基于 `CountryManager.Country` 枚举生成的。只有在该枚举中定义的国家才会出现在勋章列表中。如果只添加了图片而没有在代码中定义，图片不会显示。

### 正确的添加新国家流程：

1. **更新 `Footprint/Helpers/CountryManager.swift`**：
   - 在 `Country` 枚举中添加新国家：`case myanmar = "MM"`
   - 在 `displayName` 的 switch 中添加中文名称：`.myanmar: return "缅甸"`
   - 在 `englishName` 的 switch 中添加英文名称：`.myanmar: return "Myanmar"`
   - 在 `flag` 的 switch 中添加国旗 emoji：`.myanmar: return "🇲🇲"`

2. **准备图片文件**：
   - 将图片放入 `BadgeImages/Countries/` 文件夹
   - 可以使用中文名称（如 `缅甸.jpg`）或 ISO 代码（如 `MM.jpg`）

3. **运行导入脚本**：
   ```bash
   ./scripts/import_badge_images.sh country
   ```
   脚本会自动将中文名称映射到 ISO 代码，并在 `Assets.xcassets` 中创建 `CountryBadge_MM.imageset`
...
❌ **错误做法**：先添加图片，没有更新代码 → 图片不会显示  
✅ **正确做法**：先更新代码，再添加图片 → 正常显示
